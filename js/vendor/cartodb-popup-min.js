...
L.CartoDBPopup = L.Class.extend({
  ...
  onRemove: function(map) {
    ...
    map.off('viewreset', this._updatePosition, this).off('preclick', this._close, this);
  ...
  _close: function() {
    ...
    if (map) {
      ...
      emile(div, {
        ...
        duration: 100,
        after: function() {
          map.removeLayer(that).fire('popupclose', {
            popup: that
          });
        }
      });
...
function(a) {
  ...
  function u(a, b, c) {
    var d = 2,
      e, f, g, h = [],
      i = [];
    while ((e = 3) && (f = arguments[d - 1]) && d--) if (t(f, 0) == "r") {
      f = f.match(/\d+/g);
      while (e--) h.push(~~f[e])
    } else {
  ...
  var b = document.createElement("div"),
    ...
    j = "top bottom left right borderWidth borderBottomWidth borderLeftWidth borderRightWidth borderTopWidth borderSpacing borderRadius marginBottom marginLeft marginRight marginTop width height maxHeight maxWidth minHeight minWidth paddingBottom paddingLeft paddingRight paddingTop fontSize wordSpacing textIndent letterSpacing outlineWidth outlineOffset",
    k = (i + " " + j).split(" ");
